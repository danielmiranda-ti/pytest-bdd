...
def split_line(line):
    """Split the given table line.

    :param str|unicode line: Feature file table line.
...
def parse_table_line(line):
    ...
    :return: `string list`.
    """
    return [l for l in split_line(line)]
        ...
        with codecs.open(filename, encoding=encoding) as f:
            ...
            for line_number, line in enumerate(content.splitlines(), start=1):
                ...
                if step and (step.indent < line_indent or ((not unindented_line) and multiline_step)):
                    if is_table_line(stripped_line):
                        ...
                        if step.datatable.params:
                            ...
                            step.datatable.add_row(parse_table_line(stripped_line))
                        else:
                            step.datatable.set_param_names(parse_table_line(stripped_line))
                    else:
                        multiline_step = True
                ...
                if mode in [types.SCENARIO, types.SCENARIO_OUTLINE]:
                    ...
                    (scenario or self).examples.set_param_names(parse_table_line(parsed_line))
